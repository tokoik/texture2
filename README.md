...
このプログラムは、OpenGL を用いて 3D オブジェクト（立方体）にテクスチャ画像（画像データ）を貼り付ける「テクスチャマッピング」の基本を学ぶためのサンプルプログラムです。

通常のテクスチャマッピングとは少し異なり、スライドプロジェクターで物体に画像を投影するような「テクスチャ投影（Projective Texture Mapping）」という手法を実装しています。プログラムを実行すると、テクスチャが立方体に投影されながらアニメーションする様子を観察できます。
