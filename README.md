# texture2: テクスチャ第６回「投影マッピング」サンプルプログラム
...
今回は通常のテクスチャマッピングとは少し異なり、スライドプロジェクターやスポットライトで物体に画像を投影するような**テクスチャ投影 (Projective Texture Mapping)** という手法を実装しました。プログラムを実行すると、テクスチャが立方体に投影されながらアニメーションする様子を観察できます。
